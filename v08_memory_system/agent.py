...
def new_runtime_state(session_id: str) -> dict[str, Any]:
    return {
        ...
        "last_memory_message_index": None,
...
def validate_runtime_state(value: Any) -> dict[str, Any]:
    ...
    cursor = value["last_memory_message_index"]
    if cursor is not None and (
        isinstance(cursor, bool)
        or not isinstance(cursor, int)
        or cursor < 0
        or cursor >= len(value["messages"])
    ):
        raise StateLoadError("last_memory_message_index is invalid")
...
def classify_llm_error(exc: Exception) -> dict[str, Any]:
    ...
    prompt_too_long = status in {400, 413} and any(marker in normalized for marker in (
        "prompt_too_long", "context_length_exceeded", "maximum_context_length", "too_many_tokens",
    ))
    retryable = not prompt_too_long and (
    ...
    elif status == 429 and "model_concurrency_rate_limit_exceeded" in message:
...
def estimate_context_tokens(
    messages: list[dict[str, Any]], *, system: str = SYSTEM, tools: Any = TOOLS
) -> int:
    """Estimate the complete request input with a stable conservative ratio."""
    payload = {"system": system, "messages": _jsonable(messages), "tools": tools}
    chars = len(json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":")))
    return (chars + 2) // 3
...
def _compact_result_block(
    ...
    preview = raw[:head] + ("\n...\n" if len(raw) > preview_chars else "") + raw[-tail:]
    reference = path.resolve().as_posix()
    block["content"] = (
        f"{preview}\n[Full Tool Result: original_chars={len(raw)}; "
        f"reference={reference}]"
    )
    return len(raw), len(block["content"]), reference


def _result_messages(messages: list[dict[str, Any]]) -> list[tuple[int, list[dict[str, Any]]]]:
    found = []
    for index, message in enumerate(messages):
...
def _apply_tool_result_budget(
    ...
    if not batches:
        return processed
    _index, blocks = batches[-1]
    lengths = [len(_tool_result_text(block) or "") for block in blocks]
    total = sum(lengths)
    batch_over = total > TOOL_RESULT_BATCH_CHARS
    ...
    if active_chars > TOOL_RESULT_BATCH_CHARS:
        raise ValueError("current Tool Result batch cannot be reduced below its active payload limit")
    return processed


def _apply_microcompact(
    messages: list[dict[str, Any]], session_id: str, *, system: str = SYSTEM
) -> list[dict[str, Any]]:
    consumed = []
    ...
    for block in eligible:
        ...
        if estimate_context_tokens(messages, system=system) <= target:
            break
    return processed


def _summary_sections_valid(text: str) -> bool:
    return all(re.search(rf"(?m)^#+\s*{number}\.\s*{re.escape(title)}\s*$", text) for number, title in enumerate((
...
def _compact_boundaries(messages: list[dict[str, Any]]) -> list[int]:
    """Return legal cuts preferring two raw rounds, then one, then the minimum suffix."""
    legal = []
    for cut in range(1, len(messages)):
        if messages[cut].get("role") != "user":
            continue
        try:
            validate_llm_message_protocol(messages[:cut])
            validate_llm_message_protocol(messages[cut:])
        except ValueError:
            continue
        legal.append(cut)
    ...
    ordered = []
    for raw_rounds in (RECENT_RAW_ROUNDS, 1):
        matching = [
            cut for cut in legal
            if sum(message.get("role") == "assistant" for message in messages[cut:]) == raw_rounds
        ]
        if matching:
            ordered.append(max(matching))
    if legal:
        ordered.append(max(legal))
    return list(dict.fromkeys(ordered))


def _full_compact(
    messages: list[dict[str, Any]], logger: EventLogger, run_id: str,
    round_number: int, *, request_system: str = SYSTEM,
) -> list[dict[str, Any]]:
    boundaries = _compact_boundaries(messages)
    if not boundaries:
        raise ValueError("oversized current input: no legal conversation-history boundary is available for Full Compact")
    ...
    for attempt, cut in enumerate(boundaries, 1):
        ...
        logger.emit("context.summary.started", run_id, round=round_number, estimated_tokens=summary_estimate, attempt=attempt)
        response = client.messages.create(model=MODEL, system=SUMMARY_SYSTEM, messages=summary_messages, max_tokens=SUMMARY_MAX_TOKENS)
        content = _as_message_content(_value(response, "content"))
        summary = _text_from(content)
        if not summary or not _summary_sections_valid(summary):
            raise ValueError("summary response does not match the fixed nine-section schema")
        compacted = [{"role": "user", "content": [{"type": "text", "text": "<context_summary>\n" + summary + "\n</context_summary>"}]}] + copy.deepcopy(recent)
        validate_llm_message_protocol(compacted)
        compacted_estimate = estimate_context_tokens(compacted, system=request_system)
        logger.emit("context.summary.completed", run_id, round=round_number, sections=9, attempt=attempt,
                    estimated_tokens=compacted_estimate)
        if compacted_estimate < CONTEXT_TARGET_TOKENS:
            return compacted
    raise ValueError("Full Compact did not reach target watermark at any legal boundary")


def prepare_context(
    messages: list[dict[str, Any]], logger: EventLogger, run_id: str, round_number: int,
    runtime_state: dict[str, Any] | None, state_path: Path | None, *, force_full: bool = False,
    system: str = SYSTEM,
) -> dict[str, Any]:
    """Apply V07's ordered request preflight transactionally to active messages."""
    original = copy.deepcopy(messages)
    original_cursor = runtime_state.get("last_memory_message_index") if runtime_state is not None else None
    working = copy.deepcopy(messages)
    session_id = runtime_state["session_id"] if runtime_state else logger.session_id
    before = estimate_context_tokens(working, system=system)
    try:
        budgeted = _apply_tool_result_budget(working, session_id)
        after_budget = estimate_context_tokens(working, system=system)
        micro = (
            _apply_microcompact(working, session_id, system=system)
            if after_budget >= CONTEXT_TRIGGER_TOKENS
            else []
        )
        after_low_cost = estimate_context_tokens(working, system=system)
        full = force_full or after_low_cost >= CONTEXT_TRIGGER_TOKENS
        if full:
            compacted = _full_compact(working, logger, run_id, round_number, request_system=system)
            after_full = estimate_context_tokens(compacted, system=system)
            messages[:] = compacted
            working = compacted
            if runtime_state is not None:
                runtime_state["last_memory_message_index"] = None
                logger.emit("memory.cursor.reset", run_id, reason="context.full_compacted")
        elif budgeted or micro:
            messages[:] = working
        if budgeted or micro or full:
            checkpoint(runtime_state, state_path, logger, "context.full_compacted" if full else "context.tool_results_compacted")
        final = estimate_context_tokens(working, system=system)
        logger.emit("context.budget", run_id, round=round_number, estimated_tokens=final,
                    trigger_tokens=CONTEXT_TRIGGER_TOKENS, target_tokens=CONTEXT_TARGET_TOKENS,
                    triggered=bool(budgeted or micro or full), reason="ptl_recovery" if force_full else ("threshold" if full else "request_preflight"),
                    mechanisms=(["tool_result_budget"] if budgeted else []) + (["microcompact"] if micro else []) + (["full_compact"] if full else []),
                    before_tokens=before, after_tokens=final, tool_results_processed=len(budgeted) + len(micro),
                    tool_result_before_chars=sum(x["before_chars"] for x in budgeted + micro), tool_result_after_chars=sum(x["after_chars"] for x in budgeted + micro), summary_called=full)
        return {"full_compact": full, "estimated_tokens": final}
    except Exception:
        messages[:] = original
        if runtime_state is not None:
            runtime_state["messages"] = messages
            runtime_state["last_memory_message_index"] = original_cursor
        raise
...
def agent_loop(
    ...
    *,
    ...
    for round_number in range(start_round, max_rounds + 1):
        ...
        try:
            prepare_context(messages, logger, run_id, round_number, runtime_state, state_path, system=system)
        except Exception as exc:
            error = redact(str(exc))
            logger.emit("context.failed", run_id, round=round_number, mechanism="request_preflight", message=error)
            return _finish_agent_loop({"status": "failed", "final_text": "", "rounds": round_number, "error": "CONTEXT_COMPACT_FAILED"}, runtime_state, state_path, logger)
        try:
            validate_llm_message_protocol(messages)
        except ValueError as exc:
            error = redact(str(exc))
            logger.emit("llm.failed", run_id, model=MODEL, round=round_number, attempt=None, error_type="PROTOCOL_ERROR", error=error, message=error)
            return _finish_agent_loop({"status": "failed", "final_text": "", "rounds": round_number, "error": "PROTOCOL_ERROR"}, runtime_state, state_path, logger)
        set_interruption(runtime_state, state_path, logger, "llm_call")
        response = None
        ptl_recovery_used = False
        transient_retries = 0
        attempt = 0
        while response is None:
            ...
            try:
                ...
                if classified["prompt_too_long"]:
                    if ptl_recovery_used:
                        logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="failed", reason="repeated_prompt_too_long")
                        return _finish_agent_loop({"status": "failed", "final_text": "", "rounds": round_number, "error": "PROMPT_TOO_LONG"}, runtime_state, state_path, logger)
                    logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="started", reason="prompt_too_long")
                    try:
                        prepared = prepare_context(
                            messages, logger, run_id, round_number, runtime_state,
                            state_path, force_full=True, system=system,
                        )
                    except Exception as recovery_exc:
                        logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="failed", reason=redact(str(recovery_exc)))
                        return _finish_agent_loop({"status": "failed", "final_text": "", "rounds": round_number, "error": "PTL_RECOVERY_FAILED"}, runtime_state, state_path, logger)
                    if prepared["estimated_tokens"] >= CONTEXT_TRIGGER_TOKENS:
                        logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="failed", reason="unsafe_reestimate")
                        return _finish_agent_loop({"status": "failed", "final_text": "", "rounds": round_number, "error": "PTL_RECOVERY_FAILED"}, runtime_state, state_path, logger)
                    ptl_recovery_used = True
                    logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="retrying", estimated_tokens=prepared["estimated_tokens"])
                    continue
                ...
                transient_retries += 1
                delay = retry_delay(transient_retries, classified["retry_after"])
                logger.emit(
                    "llm.retry",
                    run_id,
                    model=MODEL,
        ...
        if ptl_recovery_used:
            logger.emit("context.ptl_recovery", run_id, round=round_number, attempt=1, status="succeeded")
...
def _memory_pipeline(logger: EventLogger) -> MemoryPipeline:
...
def _memory_after_completed_run(
    messages: list[dict[str, Any]],
    runtime_state: dict[str, Any] | None,
    state_path: Path | None,
    logger: EventLogger,
    ...
    """Best-effort post-processing; never changes an already completed Run."""
    pipeline = _memory_pipeline(logger)
    old_cursor = runtime_state.get("last_memory_message_index") if runtime_state is not None else None
    try:
        processed = pipeline.extract_and_store(messages, old_cursor, run_id)
        if runtime_state is not None:
            runtime_state["last_memory_message_index"] = processed["cursor"]
            try:
                checkpoint(runtime_state, state_path, logger, "memory.cursor_advanced")
            except Exception:
                runtime_state["last_memory_message_index"] = old_cursor
                raise
            logger.emit(
                "memory.cursor.advanced",
                run_id,
                previous=old_cursor,
                current=processed["cursor"],
            )
    except Exception as exc:
        logger.emit(
...
def run_once(
    ...
    *,
    ...
    recall_context = ""
    try:
        recall_context = _memory_pipeline(logger).recall(user_text, run_id).system_context
    except Exception as exc:
        logger.emit("memory.recall.failed", run_id, error_type=type(exc).__name__, message=redact(str(exc)))
    request_system = SYSTEM + recall_context
    ...
    if result["status"] == "completed":
        _memory_after_completed_run(messages, runtime_state, state_path, logger, run_id)
...
def resume_run(
    ...
    *,
    ...
    if result["status"] == "completed":
        _memory_after_completed_run(
            state["messages"], state, state_path, logger, run["run_id"]
        )
    event_type = "run.completed" if result["status"] == "completed" else "run.failed"
    final_summary = summarize(result["final_text"])
    logger.emit(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="V07 context-aware durable coding-agent runtime")
    parser.add_argument("--resume", type=Path, help="resume a persisted V07 state file")
