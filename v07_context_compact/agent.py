...
def _full_compact(messages: list[dict[str, Any]], logger: EventLogger, run_id: str, round_number: int) -> list[dict[str, Any]]:
    ...
    for attempt, cut in enumerate(boundaries, 1):
        ...
        compacted_estimate = estimate_context_tokens(compacted)
...
def prepare_context(
    ...
    try:
        ...
        if full:
            compacted = _full_compact(working, logger, run_id, round_number)
            after_full = estimate_context_tokens(compacted)
            messages[:] = compacted
            working = compacted
        ...
        final = estimate_context_tokens(working)
...
def agent_loop(
    ...
    *,
    ...
    for round_number in range(start_round, max_rounds + 1):
        ...
        try:
            prepare_context(messages, logger, run_id, round_number, runtime_state, state_path)
        except Exception as exc:
            error = redact(str(exc))
        ...
        while response is None:
            ...
            try:
                ...
                if classified["prompt_too_long"]:
                    ...
                    try:
                        prepared = prepare_context(messages, logger, run_id, round_number, runtime_state, state_path, force_full=True)
